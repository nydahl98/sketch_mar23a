...
void AD4116Class::print_byte(byte value) {
	char format[10];
	sprintf(format, "0x%.2X ", value);
	Serial.print(format);
}

int AD4116Class::set_register(adc7173_register_t reg, byte *value, int value_len) {
...
int AD4116Class::get_register(adc7173_register_t reg, byte *value, int value_len) {
	/* send communication register id 0x00 */
	SPI.transfer(0x00);
...
int AD4116Class::get_current_data_channel(adc7173_register_t &channel) {
...
int AD4116Class::set_adc_mode_config(data_mode_t data_mode, clock_mode_t clock_mode, ref_mode_t ref_mode) {
...
int AD4116Class::set_interface_mode_config(bool continuous_read, bool append_status_reg) {
...
int AD4116Class::get_data(byte *value) {
	...
	if (DEBUG_ENABLED) {
		...
		long int value0 = value[0];
		long int value1 = value[1];
		long int value2 = value[2];
		...
		long long int id3 = (value0 << 16) | (value1 << 8) | value2;




		Serial.print("id3 = ");
		Serial.println((double)id3);

		double id4 = id3 - 8387126;
		Serial.print("id4 = ");
		Serial.println(id4);
		...
		Serial.print("InputResultsScaled: ");
		Serial.println(InputResultsScaled, 6);

		long long int InputResults = 50000 * (id3 - 8387126) / 1676914;
		Serial.print("InputResults: ");
		Serial.println((int)InputResults);
...
int AD4116Class::set_channel_config(adc7173_register_t channel, bool enable, adc7173_register_t setup, analog_input_t ain_pos, analog_input_t ain_neg) {
...
int AD4116Class::set_setup_config(adc7173_register_t setup, coding_mode_t coding_mode, ain_buf_mode_t ain_buf_mode, setup_ref_source_t setup_ref_source) {
...
int AD4116Class::set_filter_config(adc7173_register_t filter, data_rate_t data_rate) {
...
int AD4116Class::set_offset_config(adc7173_register_t offset, uint32_t offset_value) {
